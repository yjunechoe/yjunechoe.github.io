...
In **{kableExtra}**, this doesn't color the third column "skyblue", the fourth column "forestgreen", and the fifth column "chocolate":
...
In **{dplyr}**, this doesn't make 3 new columns named "a", "b", and "c", all filled with NA:^[If this is your first time seeing the "bang bang" `!!` operator and the "walrus" `:=` operator being used this way, check out the [documentation on quasiquotation](https://rlang.r-lib.org/reference/quasiquotation.html). If you are familiar with quasiquation in {dplyr}, I should also mention that using "big bang" `!!!` like in `mutate(!!!new_cols := NA)` doesn't work either. As far as I know, `:=` is just an alias of `=` for the {rlang} parser, and as we know `=` cannot assign more than one variable at once (unlike Python, for example), which explains the error.]
...
So you have to do either one of these:
...
Still with me? Let's animate this!
...
```{r, echo=F, fig.align='center', layout = 'l-body-outset'}
knitr::include_graphics("table_gif.gif", error = FALSE)
```
...
You know the drill - a simple call to `reduce()` does the job!
...
Again, this affords you a lot more flexibility, like dynamically assigning values to each column:
...
We can go crazy using [context dependent expressions](https://dplyr.tidyverse.org/reference/context.html) like `cur_data()`, and do something like keeping track of the columns present at each point a new column has been created via `mutate()`:
...
We can also infer this from this output of a simple linear model:

```{r}
broom::tidy(lm(flipper_length_mm ~ species, data = penguins_implicit))
```

And you can actually make this 3-way treatment coding explicit by expanding the matrix into actual columns of the data!
...
Check this out - we get the exact same output!

```{r}
broom::tidy(lm(flipper_length_mm ~ speciesChinstrap + speciesGentoo, data = penguins_explicit))
```

By the way, if you're wondering why this is practical, some modeling packages in R (like {[lavaan](https://www.lavaan.ugent.be)} for structural equation modeling) only accept dummy coded variables that exist as independent columns/vectors, not as a metadata of a factor vector.^[Figuring this out has caused some headaches and that's what I get for not carefully reading the [docs](https://www.lavaan.ugent.be/tutorial/cat.html)] Because this is pretty common, packages like {psych} have a function that does the same transformation we just did, called `dummy.code()`^[Except `dummy.code()` also returns a column for the reference level whose value is always `1`, which is kinda pointless]:
...
And here's a {data.table} solution for explicit dummy coding as another example:
...
You can also pass in a list of functions instead of arguments because why not. 
