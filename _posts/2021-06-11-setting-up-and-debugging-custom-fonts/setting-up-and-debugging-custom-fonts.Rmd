...
One of the most practical font features is _lining_, also called `"lnum"` (the four-letter feature tag), where all numbers share the same height and baseline.
...
To access the lining feature, we can use the `systemfonts::register_variant()` function, which works similarly to `systemfonts::register_font()`. `register_variant()` is simply a wrapper around `register_font()`, and we use it here for convenience because "Alegreya" is already accessible without us having to point to the font file.
...
Then, we register the variant. Note that "ordn" is not built-in as an option for the `letters` argument of `font_features()`, whereas "lnum" from our last example was a built-in option for the `numbers` argument.^[check the help page `?systemfonts::font_feature` for details.] Therefore, we have to set the "ordn" feature inside the `...` with `"ordn" = TRUE`. And let's simultaneously also turn on the lining feature from before as well.
...
One solution is to render the month in the Regular style and the rest in the ordinal variant. We can do this with html syntax supported by `geom_richtext()` from `{ggtext}`. If you're already familiar with `{ggtext}`, this example shows that it works the same for registered custom font variants!
...
In my opinion, **you should always err towards using the supplied font features** because they are designed with the particular aesthetics of the font in mind.^[But this also means that not all fonts support "ordn", while `<sup>` is always available.] Hopefully this example has convinced you!
...
- If you're design oriented, you can also **make your own icon font** for use in R. In Inkscape, you can do this in File > New From Template > Typography Canvas ([Guide](https://inkscape-manuals.readthedocs.io/en/latest/creating-custom-fonts.html)). Once you save your SVG font, you can convert it to `.ttf` and follow the same installation process, and it should be available in R if you render with `{ragg}`.
