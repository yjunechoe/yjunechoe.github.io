...
Many different solutions already exist to LaTex math annotations. The most common is probably Non-Standard Evaluation (NSE) methods using `parse()`, `expression()`, `bquote()` etc. There are bulkier solutions like the {latex2exp} package that plots **plotmath** expressions^[Though it hasn't been updated since 2015 and it seems like there are quite a few unresolved StackOverflow issue].
...
You can also see the differences in the degree of "interpolation" by directly comparing the tables at 10 thousand vs 100 thousand samples (first vs. last frames):
