...
So to be more specific, the three calls to `select()` were the same because these three calls to `tidyselect::eval_select()` are the same:^[That last example with `quote(1)` is redundant because `1` is a constant; I kept it in there just to make the comparison clearer]
