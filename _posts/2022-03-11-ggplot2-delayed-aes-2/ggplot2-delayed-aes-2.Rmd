...
Let's pick up where we left off in [Part 1](). If you'd like to follow along without going back to re-read it, the relevant code that we'll carry over here is provided below:
...
To recap, the big theme of [Part 1]() was about how _layers_ transform the raw data under the hood to return a version of the data that is _drawing-ready_ (the data returned by `layer_data()`). We saw that a lot happens over the course of this data transformation pipeline, one of which is the **statistical transformation**. We also saw how `after_stat()` allows users to _declare_ an aesthetic that waits to be mapped to the data until after this step.

In this Part 2 of the series, we'll dig into the nitty-gritty details of the internal statistical transformation step and expose it in familiar functional programming terms. We'll work through the **What** (what are the inputs/outputs involved), and the **How** (how output is computed from the input). 
